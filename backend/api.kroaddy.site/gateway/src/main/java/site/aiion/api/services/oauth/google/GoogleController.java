    ...
    public RedirectView googleCallback(
        ...
        if (code != null) {
            try {
                ...
                // 5. Refresh Token HMAC-SHA256 해시 → User 테이블 저장 (원문은 DB에 저장 안 함)
                ...
                tokenService.clearRevoked("google", String.valueOf(appUserId));
                ...
                String redirectUrl = frontendUrl
                        + "/login/callback?provider=google"
                        + "&user_id=" + appUserId
                        + "&nickname=" + URLEncoder.encode(nickname, StandardCharsets.UTF_8);
                if (isMobileDeepLink) {
                    // 모바일 딥링크는 쿠키 사용 불가 → 토큰 URL 포함 (불가피)
