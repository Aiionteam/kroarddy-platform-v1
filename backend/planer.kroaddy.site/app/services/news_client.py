    ...
    *,
    ...
    if lang == "Korean":
        if for_k_content:
            ...
            footer = "- 뉴스에 언급된 공연·이벤트·장소가 여행지와 관련 있으면 루트 테마에 반영하세요.\n"
    else:
        if for_k_content:
            header = "【📰 Latest K-Content News – Items within travel period listed first】"
            footer = (
                "- If any news mentions performances, events, pop-up stores, or trending spots "
                "relevant to the destination, include them in the itinerary.\n"
                "- Items with 📅 (events during travel period) should be placed on the matching date.\n"
            )
        else:
            header = "【📰 Latest K-Content News (reference for route suggestions)】"
            footer = "- If news mentions performances, events, or places relevant to the destination, reflect them in route themes.\n"
