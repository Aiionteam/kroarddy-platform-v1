...
function getSeedInquiries(
  ...
  return [
    {
      id: "10001",
      title: t("customer.inquiry.seed.10001.title", { defaultValue: "로그인이 되지 않을 때" }),
      content: t("customer.inquiry.seed.10001.content", { defaultValue: "로그인 버튼을 눌러도 페이지가 로딩만 되고 로그인이 안 됩니다.\n브라우저 캐시를 지워도 동일해요." }),
      fileNames: [],
      agree: true,
    ...
    {
      id: "10002",
      title: t("customer.inquiry.seed.10002.title", { defaultValue: "공지사항 확인 경로 문의" }),
      content: t("customer.inquiry.seed.10002.content", { defaultValue: "공지 및 업데이트 내용을 어디에서 확인할 수 있나요?" }),
      fileNames: ["screenshot.png"],
      agree: true,
      createdAt: Date.now() - 1000 * 60 * 60 * 24 * 7,
      status: "answered",
      answer: t("customer.inquiry.seed.10002.answer", { defaultValue: "고객센터의 ‘공지사항’ 카테고리에서 최신 변경사항을 확인할 수 있어요." }),
    },
  ];
}
...
function InquiryContent() {
  ...
  const seedInquiries = React.useMemo(() => getSeedInquiries(t), [t]);
  ...
  const filteredInquiries = inquiries.filter((it) => {
    ...
    return (
      ...
      (it.status === "answered"
        ? t("customer.inquiry.status.answered", { defaultValue: "답변완료" })
        : t("customer.inquiry.status.pending", { defaultValue: "처리중" })
      )
        .toLowerCase()
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    alert(t("customer.inquiry.submit_done", { defaultValue: "문의가 접수되었습니다. 빠르게 답변드릴게요!" }));
  ...
  return (
    <AppLayout
      ...
      mobileTitle={
        tab === "write"
          ? t("customer.inquiry.tabs.write", { defaultValue: "1:1 문의하기" })
          : t("customer.inquiry.tabs.list", { defaultValue: "나의 문의 내역" })
      }
    >
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold text-gray-800">
                {t("customer.inquiry.title", { defaultValue: "문의하기" })}
              </h1>
            </div>
              ...
              <div className="grid gap-4">
                <label className="block">
                  <span className="mb-1 block text-sm font-semibold text-gray-700">
                    {t("customer.inquiry.form.subject", { defaultValue: "제목" })}
                  </span>
                  <input
                    type="text"
                    value={title}
                    onChange={(e) => setTitle(e.target.value)}
                    placeholder={t("customer.inquiry.form.subject_placeholder", { defaultValue: "제목을 입력하세요" })}
                ...
                <label className="block">
                  <span className="mb-1 block text-sm font-semibold text-gray-700">
                    {t("customer.inquiry.form.content", { defaultValue: "문의내용" })}
                  </span>
                  <textarea
                    value={content}
                    onChange={(e) => setContent(e.target.value)}
                    placeholder={t("customer.inquiry.form.content_placeholder", { defaultValue: "상세 내용을 입력해 주세요" })}
                    rows={8}
                ...
                <div>
                  <span className="mb-1 block text-sm font-semibold text-gray-700">
                    {t("customer.inquiry.form.attachments", { defaultValue: "첨부파일" })}
                  </span>
                  <div className="flex items-center gap-3">
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    {t("customer.inquiry.form.attachment_help", { defaultValue: "이미지 파일(GIF, PNG, JPG) 기준 최대 10MB 이하, 최대 3개 가능" })}
                  </p>
                </div>
                ...
                <label className="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 p-3">
                  ...
                  <span className="text-sm text-gray-700">
                    {t("customer.inquiry.form.privacy_agree", { defaultValue: "개인정보 수집 및 이용 동의" })}
                  </span>
                </label>
                ...
                <div className="mt-2 flex items-center justify-end gap-2">
                  ...
                  >
                    {submitting
                      ? t("customer.inquiry.form.sending", { defaultValue: "전송 중..." })
                      : t("customer.inquiry.form.submit", { defaultValue: "작성 완료" })}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="mx-auto w-full max-w-3xl px-6 py-8">
            <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <label htmlFor="inquiry-search" className="mb-2 block text-sm font-semibold text-gray-700">
                {t("customer.inquiry.list.search_label", { defaultValue: "검색" })}
              </label>
              <input
                id="inquiry-search"
                type="text"
                ...
                placeholder={t("customer.inquiry.list.search_placeholder", { defaultValue: "제목/내용/상태를 검색하세요" })}
            ...
            <section className="mt-6">
              {filteredInquiries.length === 0 ? (
                <p className="rounded-xl bg-gray-50 px-4 py-8 text-center text-sm text-gray-500">
                  {t("customer.inquiry.list.no_results", { defaultValue: "검색 결과가 없습니다." })}
                </p>
              ) : (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="shrink-0">
                          <span
                            className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${it.status === "answered"
