...
function WhisperIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 18s1-4 9-4 9 4 9 4" /><circle cx="12" cy="8" r="4" /><path d="M6 18c0-2 2.5-4 6-4s6 2 6 4" strokeDasharray="2 2" /></svg>; }
...
export const AppSidebar: React.FC<AppSidebarProps> = ({ onLogout, onClose }) => {
    ...
    <aside className="flex h-full w-64 flex-col border-r border-gray-200 bg-gray-50">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 p-4">
        ...
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1 text-gray-500 hover:bg-gray-200 md:hidden"
          ...
          {NAV.map((item) => (
            <li key={item.path}>
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm transition-colors ${isActive(item.path) ? "bg-purple-100 text-purple-800" : "text-gray-700 hover:bg-gray-100"
                  }`}
              >
                {item.icon}
      ...
      <div className="border-t border-gray-200 p-3 space-y-1">
        <button
          ...
          className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors ${pathname.startsWith("/chat/friends") ? "bg-purple-100 text-purple-800" : "text-gray-700 hover:bg-gray-100"
            }`}
        >
          <FriendsIcon />
          <span>{t("sidebar.friends", { defaultValue: "친구목록" })}</span>
        </button>
        <button
          type="button"
          onClick={() => navigate("/chat/whisper")}
          className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors ${pathname.startsWith("/chat/whisper") ? "bg-purple-100 text-purple-800" : "text-gray-700 hover:bg-gray-100"
            }`}
        >
          <WhisperIcon />
          <span>{t("sidebar.whisper", { defaultValue: "귓속말" })}</span>
        </button>
        <button
          type="button"
          onClick={() => navigate("/profile/settings")}
          className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors ${pathname.startsWith("/profile/settings") ? "bg-purple-100 text-purple-800" : "text-gray-700 hover:bg-gray-100"
            }`}
        >
          <SettingsIcon />
          <span>{t("sidebar.profile", { defaultValue: "내정보" })}</span>
        </button>
        <button
          type="button"
          onClick={() => navigate("/customer")}
          className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors ${pathname.startsWith("/customer") ? "bg-purple-100 text-purple-800" : "text-gray-700 hover:bg-gray-100"
            }`}
        >
          <span className="shrink-0 translate-y-px">
            <CustomerIcon />
          </span>
          <span>{t("sidebar.customer", { defaultValue: "고객센터" })}</span>
        </button>
        <button
          type="button"
          onClick={onLogout}
        ...
        >
          <LogoutIcon />
          <span>{t("sidebar.logout", { defaultValue: "로그아웃" })}</span>
        </button>
      </div>
    </aside>
  );
