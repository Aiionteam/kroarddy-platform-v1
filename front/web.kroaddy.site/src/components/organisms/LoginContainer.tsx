    ...
    <div className="w-full max-w-[400px]">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-gray-500 tracking-wide">
          {t("auth.tagline", { defaultValue: "AI와 함께하는 스마트 여행 플래너" })}
        </p>
      </div>

      <div className="p-0">
        <p className="text-[11px] font-semibold tracking-[0.15em] text-gray-500 uppercase text-center mb-5">
          {t("auth.social_login_heading", { defaultValue: "소셜 계정으로 시작하기" })}
        </p>
        ...
        <div className="flex items-center gap-3 my-5">
          ...
          <span className="text-[11px] text-gray-400">{t("auth.or", { defaultValue: "또는" })}</span>
          <div className="flex-1 h-px bg-gray-200" />
        </div>

        <Button type="button" variant="outline" onClick={handleGuestLogin}>
          {t("auth.guest_browse", { defaultValue: "게스트로 둘러보기" })}
        </Button>
      </div>

      <p className="text-center text-[11px] text-gray-400 mt-6 leading-relaxed">
        {t("auth.login_terms_notice", {
          defaultValue: "로그인하면 서비스 이용약관 및 개인정보처리방침에 동의하는 것으로 간주합니다.",
        })}
      </p>
    </div>
  );
